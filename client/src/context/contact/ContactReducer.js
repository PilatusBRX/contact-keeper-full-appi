export default (state, action) => {
  switch (action.type) {
    case 'GET_CONTACTS':
      return {
        ...state,
        contacts: action.contacts,
        loading: false,
      };
    case 'ADD_CONTACT':
      return {
        ...state,
        contacts: [action.contacts, ...state.contacts],
        loading: false,
      };
    case 'UPDATE_CONTACT':
      return {
        ...state,
        contacts: state.contacts.map((contact) =>
          contact._id === action.contacts._id ? action.contacts : contact
        ),
        loading: false,
      };
    case 'DELETE_CONTACT':
      return {
        ...state,
        contacts: state.contacts.filter(
          (contact) => contact._id !== action.contacts
        ),
        loading: false,
      };
    case 'CLEAR_CONTACTS':
      return {
        ...state,
        contacts: null,
        filtered: null,
        error: null,
        current: null,
      };
    case 'SET_CURRENT':
      return {
        ...state,
        current: action.contacts,
      };
    case 'CLEAR_CURRENT':
      return {
        ...state,
        current: null,
      };
    case 'FILTER_CONTACTS':
      return {
        ...state,
        filtered: state.contacts.filter((contact) => {
          const regex = new RegExp(`${action.contacts}`, 'gi');
          return (
            contact.name.match(regex) ||
            contact.email.match(regex) ||
            contact.phone.match(regex) ||
            contact.type.match(regex)
          );
        }),
      };
    case 'CLEAR_FILTER':
      return {
        ...state,
        filtered: null,
      };
    case 'CONTACT_ERROR':
      return {
        ...state,
        error: action.contacts,
      };
    default:
      return state;
  }
};
